...
def build_skill_harness_comment(diff_result, lint_results, rule_analysis, content_analysis, analysis_available=True) -> str:
    ...
    if not analysis_available:
        lines.append("---\n")
        lines.append("⚠️ LLM analysis unavailable\n")
...
def post_comment_command(diff: str, lint_report: str, pr: int, repo: str, token: str):
    ...
    # Parse lint results
    lint_report_path = Path(lint_report)
    if not lint_report_path.exists():
        click.echo(f"Error: Lint report file not found: {lint_report}", err=True)
        sys.exit(1)
